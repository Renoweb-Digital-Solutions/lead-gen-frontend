  ...
  return (
    <>
      ...
      <animated.div
        ...
        className="fixed bottom-0 left-0 right-0 z-[110] bg-white rounded-t-[32px] shadow-[0_-10px_40px_rgba(220,38,38,0.1)] flex flex-col will-change-transform"
      >
        <div
          {...bind()}
          onClick={() => { if (!isOpen && onOpen) onOpen(); }}
          className="relative w-full pt-3 pb-3 flex flex-col items-center justify-center cursor-grab active:cursor-grabbing touch-none group hover:bg-red-500/[0.05] transition-colors rounded-t-[32px]"
        >
          {/* Main Grip Handle */}
          <div className="flex items-center justify-center w-16 h-1.5 rounded-full bg-gray-200 group-hover:bg-red-500/40 transition-colors mb-2" />

          {/* Subtle icon/hint visible mainly when closed */}
          {!isOpen && (
            <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.15em] text-red-600/60 group-hover:text-red-600 transition-colors">
